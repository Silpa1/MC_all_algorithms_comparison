...
resLpre = param.A(reshape(X,[nx,ny,nt]))-param.d;
M = X - t*reshape(param.At(resLpre),[nx*ny,nt]);
YM = M; %for fista
...
%     M=X-t*reshape(param.E'*resk,[nx*ny,nt]); 
    resk=param.A(reshape(X,[nx,ny,nt]))-param.d;
    M=X-t*reshape(param.At(resk),[nx*ny,nt]); 
    ...
    if arg.fistaL && arg.fistaS
        ...
        Thetapre = Theta;
        % data consistency
        reskY=param.A(reshape(YLpre+YS,[nx,ny,nt]))-param.d;
